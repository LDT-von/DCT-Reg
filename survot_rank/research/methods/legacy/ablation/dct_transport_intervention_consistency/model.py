...
DEPRECATED: This class is retained for ablation studies only.
The paper-facing method (DCTV310DirectionalRegularizedTransport) disables all
v3.8 structural losses by setting their weights to zero.
...
---
TO USE THIS CLASS:
  → Only for ablation experiments (e.g., "what if we enable dose loss?")
  → Not for paper results (use DCTV310DirectionalRegularizedTransport instead)
---

NOTE (2026-09-09): The v3.10 audit showed DCR ≈ 0.526 ≈ random, suggesting
the direction loss provides minimal interpretability benefit. It is disabled
in the paper method. See scripts/test_ot_contribution.py for the audit code.
"""
