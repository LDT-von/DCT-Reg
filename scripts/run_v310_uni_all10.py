#!/usr/bin/env python3
"""Run DCT v3.10 DCT-Reg with UNI encoder (1024-dim) on all 10 cancers.

Recipe is identical to scripts/run_dct_v310_final_cross_cancer.py
EXCEPT encoder → UNI and splits → 5fold (UNI splits).
This is the v3.10 baseline measured under UNI features, previously missing.

Usage:
    python scripts/run_v310_uni_all10.py plan
    python scripts/run_v310_uni_all10.py doctor
    python scripts/run_v310_uni_all10.py run [--force]
    python scripts/run_v310_uni_all10.py smoke
"""
...
# All 10 supported cancers, in a stable order.
DEFAULT_CANCERS = (
    "blca", "brca", "coadread", "hnsc", "kirc",
    "luad", "lusc", "skcm", "stad", "ucec",
)
DEFAULT_FOLDS = (0, 1, 2, 3, 4)
RESULT_ROOT = Path("results/dct_v3.10_uni_all10/final")
SMOKE_ROOT = Path("results/dct_v3.10_uni_all10/smoke")

# Frozen recipe; the only differences vs the UNI2-h version are
# wsi_encoder / encoding_dim / which_splits / data_root_dir / batch_size.
# num_patches=4096 matches the v3.11/v3.12 UNI configs (UNI patches are larger).
FROZEN_OVERRIDES: dict[str, object] = {
    ...
    "dct_mix_ratio": 1.0,
    "num_patches": 4096,                           # matches v3.11/v3.12 UNI
    "batch_size": 8,
    "which_splits": "5fold",                       # UNI splits
    "on_missing_wsi": "error",
    "wsi_encoder": "uni",
    "encoding_dim": 1024,                          # UNI = 1024-dim
}
    ...
    values.update(
        {
            ...
            "specific_simple": f"dct_v310_dct_reg_uni_{cancer}_30ep",
        }
    )
    if smoke:
        values.update(
            {
                "max_epochs": 2,
                "max_smoke_batches": 2,
                "specific_simple": f"dct_v310_dct_reg_uni_smoke_{cancer}",
            }
        )
    command = (
        args.python_bin,
...
def print_plan(jobs: list[Job], *, force: bool = False, run_mode: bool = False) -> None:
    ...
    print("ENCODER:     UNI (1024-dim, num_patches=4096)")
...
def run_queue(args: argparse.Namespace, jobs: list[Job], *, smoke: bool) -> int:
    ...
    try:
        scheduler_lock = runtime.acquire_run_lock(
            ...
            label=f"DCT v3.10 UNI all10 queue on GPU {args.gpu}",
        )
        ...
        for index, job in enumerate(jobs, start=1):
            ...
            try:
                print(
                    ...
                    f"DCT v3.10 UNI {job.cancer.upper()} fold{job.fold}"
                )
                print(shlex.join(job.command))
