...
# Key: num_patches=4096 (from config, NOT overridden)

LOG_BASE="/data1/DCT-Reg/logs/v313_5fold_p4096_aligned"
RESULT_DIR="/data1/DCT-Reg/results/dct_v313_blca_uni_p4096_5fold_aligned/blca"
...
echo "============================================"
echo "BLCA v3.13 UNI — 5-Fold Sequential (GPU 0, p4096)"
echo "Start: $(date '+%Y-%m-%d %H:%M:%S')"
echo "Config: ${CONFIG}  (num_patches=4096)"
echo "============================================"
...
for fold in $(seq $START_FOLD 4); do
    ...
    # NOTE: NOT overriding num_patches — let config's 4096 take effect
    "${PYTHON}" -m survot_rank.cli train \
        ...
        --set k_end=$((fold + 1)) \
        --set gpu=0 \
        --set specific_simple="dct_v313_blca_uni_p4096_5fold_aligned" \
...
VALUES=$(for fold in 0 1 2 3 4; do
    grep -E "best cindex|best c-index" "/data1/DCT-Reg/logs/v313_5fold_p4096_aligned/fold${fold}/fold${fold}.log" 2>/dev/null | head -1 | sed 's/.*cindex=\([0-9.]*\).*/\1/'
done)
