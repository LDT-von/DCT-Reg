...
echo "============================================"
echo "BLCA v3.12 UNI — 5-Fold Sequential (GPU 0)"
...
VALUES=$(for fold in 0 1 2 3 4; do
    grep -E "best cindex|best c-index" "/data1/DCT-Reg/logs/v312_5fold_aligned/fold${fold}/fold${fold}.log" 2>/dev/null | head -1 | sed 's/.*cindex=\([0-9.]*\).*/\1/'
done)
