...
def run_queue(args: argparse.Namespace, jobs: list[Job], *, smoke: bool) -> int:
    if base.runtime.doctor(args):
        return 2
    environment = os.environ.copy()
    environment["CUDA_VISIBLE_DEVICES"] = str(args.gpu)
    environment["CUDA_DEVICE_ORDER"] = "PCI_BUS_ID"
    environment.setdefault("PYTHONUNBUFFERED", "1")
    if not base.runtime.verify_child_cuda(args.python_bin, environment):
        return 1

    scheduler_lock = None
    try:
        scheduler_lock = base.runtime.acquire_run_lock(
            base.runtime.scheduler_lock_path(args.gpu, smoke),
            label=f"DCT v3.10 matched experiments on GPU {args.gpu}",
        )
    except base.runtime.ActiveRunError as error:
    ...
    try:
        for index, job in enumerate(jobs, start=1):
            ...
            task_lock = None
            try:
                task_lock = base.runtime.acquire_run_lock(
                    base.runtime.task_lock_path(job),
                    label=(
            ...
            try:
                ...
                base.runtime.release_run_lock(task_lock)
        return 0
    finally:
        base.runtime.release_run_lock(scheduler_lock)
...
def main() -> int:
    ...
    if args.mode == "doctor":
        return base.runtime.doctor(args)
